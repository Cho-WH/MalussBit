...
export const startSweep = async ({ fromDeg, toDeg, durationMs }) => {
  const duration = Number(durationMs)
  const parts = [
    'SWEEP',
    Number(fromDeg).toFixed(1),
    Number(toDeg).toFixed(1),
    Number.isFinite(duration) ? duration : 0,
  ]
  await writeLine(parts.join(','))
}

export const stopSweep = async () => writeLine('STOP')

export const setServoSpeed = async (speedDegPerSec) => {
  await writeLine(`CONFIG,SERVO_SPEED,${Number(speedDegPerSec).toFixed(1)}`)
}
